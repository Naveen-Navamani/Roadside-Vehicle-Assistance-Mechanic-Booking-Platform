...
export const MapView = ({ customerPoint, mechanicPoint, fleetMechanics = [], activeBooking, zoom = 13, height = '420px', onMapClick }) => {
    ...
    useEffect(() => {
        ...
        fleetMechanics.forEach((m) => {
            ...
            if (mechanicPoint && Math.abs(m.location.lat - mechanicPoint.lat) < 0.0001)
                return;
    ...
    return (_jsx("div", { className: "relative w-full rounded-2xl overflow-hidden border border-slate-800 shadow-2xl bg-slate-900", style: { height }, children: _jsx("div", { ref: mapContainerRef, className: "w-full h-full" }) }));
};
